/* myserver.c 
BIF3C1 Josef Koch if18b061
*/

#include <sys/types.h>
#include <sys/socket.h>
#include <sys/stat.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <unistd.h>
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <dirent.h>
#include <iostream>
#include <vector>
#include <fstream>
#include "myhelper/myhelper.h"
#include "mysocket/mysocket.h"

using namespace std;

int main(int argc, char *argv[])
{
   MyHelper helper;
   helper.checkServerArguments(argc, argv);

   MySocket serverSocket(stoi(argv[1]));
   serverSocket.createSocket();
   serverSocket.bindSocket();
   serverSocket.listenSocket();

   int new_socket;
   string command, buffer, argDir = argv[2];

   while (1)
   {
      cout << "Waiting for connections..." << endl;
      new_socket = serverSocket.acceptNewConnection();

      do
      {
         command = serverSocket.recvMessageFromClient(new_socket);

         if (command == "SEND\n")
         {
            // read mail data from socket
            string sender = serverSocket.recvMessageFromClient(new_socket);
            string receiver = serverSocket.recvMessageFromClient(new_socket);
            string subject = serverSocket.recvMessageFromClient(new_socket);
            string message = serverSocket.recvMessageFromClient(new_socket);

            // read message part of mail till .\n accours
            while (1)
            {
               buffer = serverSocket.recvMessageFromClient(new_socket);
               if (buffer == ".\n")
               {
                  break;
               }
               message += buffer;
            }

            // check for length violations, +1 because of \n
            if (sender.length() > 9 || receiver.length() > 9 || subject.length() > 81)
            {
               serverSocket.sendMessageToClient("ERR\n\0", new_socket);
            }
            else
            {
               // write file with data in directory of user
               string fullDir = argDir + '/' + sender.substr(0, sender.length() - 1);
               int mailId;

               // check if directory exists, create if not
               if (access(fullDir.c_str(), F_OK) == -1)
               {
                  mkdir(fullDir.c_str(), 0700);
                  cout << "created directory " << fullDir << endl;
               }

               // get number of files in directory, + 1 for new index
               mailId = helper.filesInDirectory(fullDir) + 1;
               string filename = fullDir + '/' + to_string(mailId) + '_' + subject.substr(0, subject.length() - 1) + ".txt";

               // create file write mail data to file
               ofstream file;
               file.open(filename.c_str(), ios::out);
               file << sender + receiver + subject + message + ".\n\0";
               cout << "Mail " << subject.substr(0, subject.length() - 1) << " from user "
                    << sender.substr(0, sender.length() - 1) << " safed!" << endl;
               serverSocket.sendMessageToClient("OK\n\0", new_socket);
               file.close();
            }
         }
         else if (command == "LIST\n")
         {
            string user = serverSocket.recvMessageFromClient(new_socket);
            string dirPath = argDir + '/' + user.substr(0, user.length() - 1);

            // check if user exists
            if (access(dirPath.c_str(), F_OK) == -1)
            {
               buffer = "0 mails for user " + user;
               serverSocket.sendMessageToClient(buffer.c_str(), new_socket);
            }
            else
            {
               vector<string> mails;

               // get every mail subject from directory
               mails = helper.subjectsInDirectory(dirPath);
               buffer = to_string(mails.size()) + " mails for user " + user;
               serverSocket.sendMessageToClient(buffer.c_str(), new_socket);

               // at least 1 message lies in direcotry -> send list of messages to client
               if (mails.size() > 0)
               {
                  cout << "Sent list of mails from user " << user.substr(0, user.length() - 1) << " to client!" << endl;
                  for (unsigned int i = 0; i < mails.size(); i++)
                  {
                     // check if client is ready to recv message, without this check weird things happen
                     buffer = serverSocket.recvMessageFromClient(new_socket);
                     if (buffer == "OK")
                     {
                        serverSocket.sendMessageToClient(mails[i].c_str(), new_socket);
                     }
                  }
               }
            }
         }
         else if (command == "READ\n")
         {
            string user = serverSocket.recvMessageFromClient(new_socket);
            string mailNumber = serverSocket.recvMessageFromClient(new_socket);
            ofstream file;

            user = user.substr(0, user.length() - 1);

            if (helper.stringIsInt(mailNumber))
            {
               unsigned int mailIndex = stoi(mailNumber.substr(0, mailNumber.length() - 1)) - 1;
               string line, fulldir, dirPath = argDir + '/' + user;
               ifstream file;
               vector<string> mails;

               // check if user exists
               if (access(dirPath.c_str(), F_OK) == -1)
               {
                  serverSocket.sendMessageToClient("ERR\0", new_socket);
               }
               else
               {
                  // get every file from directory
                  mails = helper.filenamesInDirecotry(dirPath);

                  // check if requested mail actually exists
                  if (mailIndex < mails.size())
                  {
                     fulldir = dirPath + '/' + mails[mailIndex];
                     file.open(fulldir.c_str());
                     if (file.is_open())
                     {
                        cout << "Sent email " << mails[mailIndex] << " informaiton from user " << user << " to client!" << endl;
                        serverSocket.sendMessageToClient("OK\0", new_socket);
                        while (getline(file, line))
                        {
                           line = line + '\n';
                           // check if client is ready to recv message, without this check weird things happen
                           buffer = serverSocket.recvMessageFromClient(new_socket);
                           if (buffer == "OK")
                           {
                              serverSocket.sendMessageToClient(line.c_str(), new_socket);
                           }
                        }
                     }
                     else
                     {
                        serverSocket.sendMessageToClient("ERR\0", new_socket);
                     }
                     file.close();
                  }
                  else
                  {
                     serverSocket.sendMessageToClient("ERR\0", new_socket);
                  }
               }
            }
         }
         else if (command == "DEL\n")
         {
            string user = serverSocket.recvMessageFromClient(new_socket);
            string mailNumber = serverSocket.recvMessageFromClient(new_socket);
            ofstream file;

            user = user.substr(0, user.length() - 1);

            if (helper.stringIsInt(mailNumber))
            {
               unsigned int mailIndex = stoi(mailNumber.substr(0, mailNumber.length() - 1)) - 1;
               string fulldir, dirPath = argDir + '/' + user;
               ifstream file;
               vector<string> mails;

               // check if user exists
               if (access(dirPath.c_str(), F_OK) == -1)
               {
                  serverSocket.sendMessageToClient("ERR\0", new_socket);
               }
               else
               {
                  // get every file from directory
                  mails = helper.filenamesInDirecotry(dirPath);

                  // check if requested mail actually exists
                  if (mailIndex < mails.size())
                  {
                     fulldir = dirPath + '/' + mails[mailIndex];

                     // delete mail txt file
                     if (remove(fulldir.c_str()) == 0)
                     {
                        cout << "Deleted mail " + mails[mailIndex] << endl;
                        serverSocket.sendMessageToClient("OK\0", new_socket);
                     }
                     else
                     {
                        serverSocket.sendMessageToClient("ERR\0", new_socket);
                     }
                  }
                  else
                  {
                     serverSocket.sendMessageToClient("ERR\0", new_socket);
                  }
               }
            }
         }

      } while (command != "QUIT\n");
      close(new_socket);
   }
   return EXIT_SUCCESS;
}
